...
test_that("submit_code auto-initializes queue when needed", {
  skip_if_not_installed("callr")
  taskr::shutdown_queue()
  taskr::submit_code({ 1 + 1 }, resources = list(slots = 1L))
...
test_that("submit_code import = auto captures globals from expression", {
  skip_if_not_installed("callr")
  taskr::shutdown_queue()
  taskr::init_queue(max_slots = 2)
  on.exit(taskr::shutdown_queue(), add = TRUE)
...
test_that("submit_code import = none disables context injection", {
  skip_if_not_installed("callr")
  taskr::shutdown_queue()
  taskr::init_queue(max_slots = 2)
  on.exit(taskr::shutdown_queue(), add = TRUE)
...
test_that("submit_code import list supports explicit context fields", {
  skip_if_not_installed("callr")
  taskr::shutdown_queue()
  taskr::init_queue(max_slots = 2)
  on.exit(taskr::shutdown_queue(), add = TRUE)
...
test_that("submit_code validates output values", {
  taskr::shutdown_queue()
  taskr::init_queue(max_slots = 2)
  on.exit(taskr::shutdown_queue(), add = TRUE)

  expect_error(taskr::submit_code(expr = { 1 }, output = "some"), "one of: \"all\" or \"none\"")
...
test_that("submit_task accepts field filtering output", {
  skip_if_not_installed("callr")
  taskr::shutdown_queue()
  taskr::init_queue(max_slots = 2)
  on.exit(taskr::shutdown_queue(), add = TRUE)
...
test_that("submit_task import = auto can capture globals from function body", {
  skip_if_not_installed("callr")
  taskr::shutdown_queue()
  taskr::init_queue(max_slots = 2)
  on.exit(taskr::shutdown_queue(), add = TRUE)
